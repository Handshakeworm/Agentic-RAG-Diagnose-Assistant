...
- **Design points**:
  ...
  - **【Deferred】 chunk-informed selection (pending eval validation)**: the bullet above ("no longer calls candidate_chunks") is the current trade-off — ④ runs right after ③ retrieve and `candidate_chunks` is already in state, but ④ doesn't read it, selecting "which HPI dimension is empty" rather than "which symptom discriminates the top differential diagnoses". Upgrade path: take the **full text of the top-5~8 distinct parent chunks in RRF order** (reusing ⑩ Step 0.5's child→parent expansion + parent dedup, `lookup_chunk_content`; reuse ④'s existing `deepseek-v4-pro` call, **no new LLM call site**) and inject into the prompt, turning selection differential-driven; no new node, doesn't touch the follow-up hard caps. **Why full text not summary**: discriminating features are granular (e.g., "aggravated after fatty meals / Murphy's sign"); the summary, designed for recall, abstracts them away — enough to pick candidate diseases but not enough to pick the discriminating symptom; and grounding ④ in the literature (rather than the LLM's parametric memory) is consistent with ⑩'s anti-hallucination line. **Why K cut to 5~8 rather than aligning with ⑩'s 20**: full text is information-dense, the top few distinct parents already cover the main candidates + discriminators; and ④ runs every round in the follow-up loop, so feeding full text ×20× per round ≈ repeatedly burning a diagnosis-sized payload, hence few-but-full. **Prerequisite**: the current single-primary-diagnosis eval set structurally cannot measure this gain (no "two diseases separated by one symptom" cases), so first build a "differential-ambiguity" eval subset proving ④'s gap-filling does miss questions, then act; the exact K (5 or 8) to be decided by measurement. Constraints: if persisting the injected literature context, the new field must carry a default per §9.2 + refresh each round of the differential loop; the access scope aligns with ⑩ using RRF-order top-K (reranker off by default), reusing parent dedup.
...
- **Responsibilities**:
  ...
  **Step 0.5: Parent-chunk expansion + same-section figures/tables** (non-LLM, Small-to-Big, 3.2.3)
  ...
  - **【Planned, see `pending tasks.md` #1】**: the parent-chunk text is currently assembled per small-chunk index and **not deduplicated by `parent_chunk_id`** (when the top-K small chunks share a parent, the same parent-chunk full text is fed to the LLM more than once, violating §3.2.3's "dedup by chunk_id" intent). Planned change: move truncation from the small-chunk level to the **parent-chunk level** — walk the small chunks in order, expanding+deduping parents on the fly, stopping once `DIAGNOSE_PARENT_TOP_K=20` distinct parent chunks are collected; the diagnose eval must be re-run afterward to confirm top-1 does not regress.
